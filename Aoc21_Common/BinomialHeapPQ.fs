...
//  type 'a treeElement = Element of uint32 * 'a
  type 'a treeElement = class val k:int val v:'a new(k,v) = { k=k;v=v } end

  type 'a tree = Node of int * 'a treeElement * 'a tree list
  ...
  let private mergeTree (Node(r,kv1,ts1) as tree1) (Node (_,kv2,ts2) as tree2) =
    if kv1.k > kv2.k then Node(r+1,kv2,tree1::ts2)
    else Node(r+1,kv1,tree2::ts1)
  ...
  let insert k v = let kv = treeElement(k,v) in let nn = Node(0,kv,[])
